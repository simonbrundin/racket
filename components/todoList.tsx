      ...
      switch (e.key) {
        case "j":
          setSelectedIndex((prev) =>
            prev === null ? 0 : Math.min(todos.length - 1, prev + 1),
          );
          break;
        case "k":
          setSelectedIndex((prev) =>
      ...
      >
        ...
        >
          <div className="flex flex-col items-start space-y-2 mb-4">
            {visibleTodos
              .filter((todo) => todo.parentIds.length === 0)
              .map((todo, index) => {
                if (editingId === todo.id) {
                  return (
                    <div
                      key={`edit-${todo.id}`}
                      className="flex flex-col p-2 rounded w-full bg-gray-100 dark:bg-gray-800 mb-2"
                    >
